$(document).ready(function () {
   ...
   var helpers = (typeof window.Helpers !== 'undefined') ? new window.Helpers() : null;
   function tr(key) {
      return (helpers && helpers.translate && typeof helpers.translate.content === 'function')
         ? helpers.translate.content(key) : (key === 'tr_c8775dcb5146e06356e352b69bb89764' ? 'Выбрано' : key === 'tr_996b125bc9bba860718d999df2ecc61d' ? 'От' : key === 'tr_538dc63d3c6db1a1839cafbaf359799b' ? 'до' : key === 'tr_c2aa9c0cecea49717bb2439da36a7387' ? 'До' : key);
   }
   ...
   $(document).on('click','.uni-select-content-item', function () {
      ...
      if(input.attr("type") == "radio"){
         ...
         if(!count){
            ...
            name.html(tr("tr_c8775dcb5146e06356e352b69bb89764")+" ("+count+")");
         }else{
   ...
   $(document).on('input','.uni-select-content-input input', function () {
      ...
      if(parent.find("input[data-type=from]").val() && parent.find("input[data-type=to]").val()){
         name.html(tr("tr_996b125bc9bba860718d999df2ecc61d")+" "+parent.find("input[data-type=from]").val()+" "+tr("tr_538dc63d3c6db1a1839cafbaf359799b")+" "+parent.find("input[data-type=to]").val());
      }else if(parent.find("input[data-type=from]").val()){
         name.html(tr("tr_996b125bc9bba860718d999df2ecc61d")+" "+parent.find("input[data-type=from]").val());
      }else if(parent.find("input[data-type=to]").val()){
         name.html(tr("tr_c2aa9c0cecea49717bb2439da36a7387")+" "+parent.find("input[data-type=to]").val());
