...
$(document).ready(function () {
   ...
   $(document).on('click','.link-geo-item', function (e) {
      ...
      if($(_this).data("purpose") == "all"){
         ...
         helpers.request({
            url:"geo-change",
            data: {id: 0, purpose: "all"},
            error: function () { finishAllCities(); }
         }, function () {
            finishAllCities();
         });
   ...
   function apply(){
      ...
      helpers.request({url:"geo-change-options", data: helpers.paramsForm('.live-filters, .ad-search-live-filters, .modal-geo-form')+"&url="+encodeURIComponent(window.location.pathname)}, function(data) {
