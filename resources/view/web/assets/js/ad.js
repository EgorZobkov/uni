...
$(document).ready(function () {
  ...
  var checkVerifyPhoneIntervalId; 
   ...
   $(document).on('submit','.form-ad-search-create', function (e) {

      $('.form-label-error').hide();

      const button = $('.adSearchActionPublication');
      helpers.startProcessLoadButton(button);

      helpers.request({
         url:"ad-search-publication",
         data: $(".form-ad-search-create").serialize(),
         precheck: true,
         button: button,
         error: function () {
            helpers.showNotice('error', 'Не удалось отправить данные. Попробуйте ещё раз или обратитесь в поддержку.');
         ...
         if(data && data["status"] == true){
            ...
            if(data && data["auth"] === false){
                ...
                helpers.formNoticeManager($('.form-ad-search-create'), data);
            }else{
                helpers.showNotice('error', 'Неизвестная ошибка при отправке данных.');
            }
   ...
   $(document).on('submit','.form-ad-search-update', function (e) {

      $('.form-label-error').hide();

      const button = $('.adSearchActionUpdate');
      helpers.startProcessLoadButton(button);

      helpers.request({
         url:"ad-search-update",
         data: $(".form-ad-search-update").serialize(),
         precheck: true,
         button: button,
         error: function () {
            helpers.showNotice('error', 'Не удалось отправить данные. Попробуйте ещё раз или обратитесь в поддержку.');
         ...
         if(data && data["status"] == true){
            ...
            if(data && data["auth"] === false){
                ...
                helpers.formNoticeManager($('.form-ad-search-update'), data);
            }else{
                helpers.showNotice('error', 'Неизвестная ошибка при отправке данных.');
            }
