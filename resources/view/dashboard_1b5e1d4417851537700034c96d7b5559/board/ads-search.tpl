  ...
  <div class="col-12 col-md-12">
    ...
    <ul class="nav nav-pills mb-3" role="tablist">
        ...
        <li class="nav-item" role="presentation">
          <a href="{{ $template->router->getRoute('dashboard-ads-search') }}" class="nav-link {% if(!isset($_POST['filter']['status'])){ echo 'active'; } %}">
      ...
      {% foreach ($template->component->ads->allStatuses() as $key => $value): %}
        ...
        <li class="nav-item" role="presentation">
          <a href="{{ requestBuildVars(['filter'=>['status'=>$value['status']]]) }}" class="nav-link {% if(isset($_POST['filter']['status'])){ if($value['status'] == $_POST['filter']['status']){ echo 'active'; } } %}">
            ...
            <span class="badge badge-auto bg-label-primary rounded-pill badge-center ms-1">{{ $template->component->ads_search_counter->getCountByStatus($value['status']) }}</span>
          </a>
        </li>
    ...
    <div class="card">
      ...
      <form class="formControlFilters">
              ...
              <div class="control-filters-container-item">
                <div class="btn-group">
                    <select class="form-select selectpicker formControlDefaultFilters" name="output">
                      <option value="100" {% if($_POST['output'] == 100){ echo 'selected=""'; } %}>Выводить по 100</option>
                      <option value="150" {% if($_POST['output'] == 150){ echo 'selected=""'; } %}>Выводить по 150</option>
                      <option value="300" {% if($_POST['output'] == 300){ echo 'selected=""'; } %}>Выводить по 300</option>
                      <option value="500" {% if($_POST['output'] == 500){ echo 'selected=""'; } %}>Выводить по 500</option>
                      <option value="1000" {% if($_POST['output'] == 1000){ echo 'selected=""'; } %}>Выводить по 1000</option>
                    </select>
                </div>
              </div>
              ...
              <div class="control-filters-container-item">
                <div class="btn-group">
                    <div class="input-group input-group-merge">
                      ...
                      <input type="text" class="form-control formControlDefaultFilters" placeholder="Поиск..." name="search" value="{{ $_POST['search'] }}">
                    </div>
                </div>
              </div>
              ...
              <div class="control-filters-container-item">
                <div class="btn-group">
                  ...
                  <ul class="dropdown-menu width100">
                    <li><a class="dropdown-item" href="{{ $template->router->getRoute('dashboard-import-export').'?table=ads_search' }}">{{ translate("tr_20ee8ea20203fd1ea3fbc9d120467ec5") }}</a></li>
                  </ul>
                </div>
              </div>
        ...
        {{ $template->ui->tpl('board/ads-search/control-filters.tpl')->modal("controlFilters", "small"); }}

      </form>

      {% component board/ads-search/list.tpl %}
