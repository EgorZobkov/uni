...
public function buildParamsForm($params=[], $category_id=0, $only_default_filters=true){
    ...
    <?php if($app->settings->active_countries){ ?>
        <?php
            ...
            if(isset($app->user) && method_exists($app->user, 'isAuth') && $app->user->isAuth() && isset($app->user->data) && isset($app->user->data->city_id) && $app->user->data->city_id){
        ...
        <div class="params-form-item params-form-item-geo-sale-city" >
            <label class="params-form-item-label" >Город продажи</label>
            ...
            <div class="uni-select params-form-item-geo-sale-city-select" data-status="0" data-bs-target="#geoModal" data-bs-toggle="modal">
                <span class="uni-select-name" data-default-name="<?php echo translate('tr_9a73b1e5b44bee481ab175b7e327451e'); ?>">
                    <?php echo $geo ? translateFieldReplace($geo->data, "name") : translate("tr_9a73b1e5b44bee481ab175b7e327451e"); ?>
                </span>
            ...
            <?php if(!$geo && $myCityUrl){ ?>
                <div class="params-form-item-geo-sale-city-my">
                    <a href="#" class="params-form-link-my-city actionSearchMyCity" data-geo-alias="<?php echo $myCityUrl; ?>">Искать в моем городе</a>
                </div>
            <?php } elseif($geo && $geo->city_id){ ?>
                <div class="params-form-item-geo-sale-city-delivery">
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="filter-city-with-delivery" name="filter[switch][city_with_delivery]" value="1" <?php echo $params["filter"]["switch"]["city_with_delivery"] ? 'checked=""' : ''; ?> >
                        <label class="form-check-label" for="filter-city-with-delivery">В том числе с пересылкой</label>
                    </div>
                </div>
            <?php } ?>
        </div>
    ...
    <div class="params-form-item params-form-item-switch" >
        <label class="switch">
          <input type="checkbox" class="switch-input" name="filter[switch][delivery]" value="1" <?php echo ($params["filter"]["switch"]["delivery"] && !$params["filter"]["switch"]["city_with_delivery"]) ? 'checked=""' : ''; ?> >
          <span class="switch-toggle-slider">
            <span class="switch-on"></span>
            <span class="switch-off"></span>
          </span>
...
public function buildQuery($request=[], $category_id=0, $geo=[]){
    ...
    if(!isset($filter["switch"]) || !is_array($filter["switch"])){
        $filter["switch"] = [];
    }

    $price_from = (int)($filter["price_from"] ?? 0);
    $price_to = (int)($filter["price_to"] ?? 0);
    ...
    if(!empty($request["search"])){
        ...
        $searchRaw = trim((string)$request["search"]);
        if($searchRaw !== ""){

            $data = $app->component->search->splitKeywords($searchRaw);
            $split = [];
            if(!empty($data["split"])){
                foreach ($data["split"] as $w) {
                    $w = trim((string)$w);
                    if($w !== ""){
                        $split[] = $w;
                    }
            ...
            if($split){
                ...
                if($app->settings->search_allowed_text){
                    $itemQuery = $app->component->search->buildKeywordsFields($split, ["search_tags", "title", "text", "article_number"]);
                }else{
                    $itemQuery = $app->component->search->buildKeywordsFields($split, ["search_tags", "title", "article_number"]);
                }
                ...
                $like = '%'.$searchRaw.'%';
                if($app->settings->search_allowed_text){
                    $query[] = "(title LIKE ? OR search_tags LIKE ? OR text LIKE ? OR article_number LIKE ?)";
                    $params[] = $like;
                    $params[] = $like;
                    $params[] = $like;
                    $params[] = $like;
                }else{
                    $query[] = "(title LIKE ? OR search_tags LIKE ? OR article_number LIKE ?)";
                    $params[] = $like;
                    $params[] = $like;
                    $params[] = $like;
                }
    ...
    if($filter["switch"]["delivery"] && !$filter["switch"]["city_with_delivery"]){
    ...
    if($geo){
        ...
        if($filter["switch"]["city_with_delivery"] && $geo->city_id){
            // Объявления в выбранном городе + любые города с возможной пересылкой
...
public function currentAliases(){
    ...
    if($request){
        ...
        if($geo){
            return outLink('i/' . $geo->alias);
        }else{
            return outLink('i/all');
        }
