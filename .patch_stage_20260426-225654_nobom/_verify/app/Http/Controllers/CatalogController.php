...
{   
    if($main_request === null || trim((string)$main_request) === ''){
        $main_request = 'all';
    }
    ...
    if($url){
        ...
        if($filter_link){
            ...
            if($geo){
                ...
                $baseLink = $this->component->geo->replaceAllCities();

                if($params){
                    return json_answer(["link"=>$baseLink.'?'.http_build_query($params)]);
                }else{
                    return json_answer(["link"=>$baseLink]);
                }
