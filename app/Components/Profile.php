...
/**
 * Список объявлений «ищу товар» пользователя для страницы профиля (до 10 шт., без пагинации).
 */
public function getAllAdsSearchUser($user_id = 0){
    global $app;

    $content = '';
    $getAds = $app->db->getAll(
        'select * from `uni_ads_search_data` where user_id=? order by id desc limit 10',
        [$user_id]
    );
    ...
    if ($getAds) {
        foreach ($getAds as $value) {
            $value = $app->component->ads->getDataByValue($value);
            $content .= $app->view->setParamsComponent(['value' => (object)$value])->includeComponent('items/profile-ads-search-list.tpl');
        }
    }
...
/**
 * Превью объявлений «ищу товар» на главной странице профиля (/profile), до 4 шт., сетка как у обычных объявлений.
 */
public function getHomeAdsSearchUser($user_id = 0){
    global $app;

    $content = '';
    $getAds = $app->db->getAll(
        'select * from `uni_ads_search_data` where user_id=? order by id desc limit 4',
        [$user_id]
    );
    ...
    if ($getAds) {
        foreach ($getAds as $value) {
            $value = $app->component->ads->getDataByValue($value);
            $content .= $app->view->setParamsComponent(['value' => (object)$value])->includeComponent('items/home-profile-search-grid.tpl');
        }
    }
...
/**
 * Активные объявления «ищу товар» пользователя для публичной карточки (/user/card/…), до 4 шт.
 */
public function getHomeAdsSearchUserPublicActive($user_id = 0){
    global $app;

    $content = '';
    $getAds = $app->db->getAll(
        'select * from `uni_ads_search_data` where user_id=? and status=? and block_forever_status=0 order by id desc limit 4',
        [$user_id, 1]
    );
...
public function outFaceCard($data=[]){
    ...
    $userId = 0;
    if (is_object($data) && isset($data->user) && !empty($data->user->id)) {
    ...
    $user = $app->model->users->cacheKey(["id"=>$userId])->find('id=?', [$userId]);
          ...
          <a class="user-item-container" href="<?php echo outRoute('profile-statistics'); ?>?item_id=<?php echo $item["id"]; ?>" >
              <div class="user-item-container-box1" >
                 <div class="user-item-container-image" >
                    <img src="<?php echo $app->component->ads->getMedia($item["media"])->images->first_m ?: $app->component->ads->getMedia($item["media"])->images->first; ?>" class="image-autofocus" >
                 </div>
              </div>
