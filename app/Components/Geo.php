...
/**
 * Сброс гео в сессии и cookie «geo» с теми же атрибутами, что и при записи (_setcookie),
 * иначе Secure-cookie не перезаписывается и setUserChange() снова подставляет старый город.
 */
public function clearGeoSessionAndCookie(){
    global $app;
...
/**
 * Каталог /ad/search: в POST модалки приходит geo_alias ссылки «Все города» (путь …/all) — сбрасываем гео (дублирует geo-change при сбое или рассинхроне).
 */
public function applyAdsSearchCatalogGeoResetIfAllAlias($raw){
    $raw = trim((string) $raw);
    if ($raw === '') {
        return;
    }
    ...
    if ($path !== '' && preg_match('#(^|/)all$#u', $path)) {
        $this->clearGeoSessionAndCookie();
    }
}
...
public function setChange($id=0, $purpose=null){
    ...
    if($purpose == "city"){
        ...
        /* «all», неизвестный purpose или сброс без валидной сущности */
        $this->clearGeoSessionAndCookie();
    }
