...
/**
 * Уникальная ссылка на карточку объявления о продаже (uni_ads_data):
 * — В БД в поле alias хранится только «чистый» slug заголовка (без суффикса -id).
 * — В URL всегда добавляется суффикс -{id}, чтобы даже при совпадении slug разные объявления имели разные адреса.
 * — Цепочка категорий: chain_build_alias_dash — алиасы предков через дефис (как в каталоге).
 * — Если у объявления выбран город (geo), первым сегментом пути идёт alias города, затем цепочка, затем {alias}-{id}.
 * — Итог: [язык/] i/{geo.alias}/ {chain}/ {alias}-{id}  или  [язык/] i/{chain}/ {alias}-{id}
 * Маршрут: GET /i/:all/:string-:int → AdCardController@card.
 */
public function buildAliasesAdCard($data=[]){
    ...
    if($data->geo){
        return outLink('i/' . $data->geo->alias . '/' . $chain->chain_build_alias_dash . '/' . $data->alias . '-' . $data->id);
    }else{
        return outLink('i/' . $chain->chain_build_alias_dash . '/' . $data->alias . '-' . $data->id);
    }
    
}

/**
 * Сегмент URL для объявлений на поиск (отличает их от продажи на том же маршруте /:all/:string-:int).
 * При необходимости задайте в настройках board_ads_search_url_segment (латиница/цифры/дефис).
 */
public function getAdsSearchUrlSegment(){
    global $app;
    $custom = isset($app->settings->board_ads_search_url_segment) ? $app->settings->board_ads_search_url_segment : null;
    if ($custom !== null && $custom !== '') {
        return slug((string)$custom) ?: 'poisk';
    }
    return 'poisk';
}

/**
 * Цепочка категории для объявления на поиск (category_id может быть 0).
 */
public function getCategoryChainDashForSearchAd($category_id=0){
    global $app;
    $id = (int)$category_id;
    if ($id && isset($app->component->ads_categories->categories[$id])) {
        return $app->component->ads_categories->chainCategory($id)->chain_build_alias_dash;
    }
    return '';
}

/**
 * Нормализует slug для хвоста URL: в БД может быть legacy «slug-id» — в ссылке не дублируем id.
 */
public function normalizeAdSearchAliasForUrl($data){
    if (!is_object($data) || empty($data->id)) {
        return '';
    }
    $id = (int)$data->id;
    $slug = isset($data->alias) ? trim((string)$data->alias) : '';
    if ($slug === '') {
        // Пустой alias в БД или после slug() — иначе outLink('') ведёт на главную
        return 'a' . $id;
    }
    if ($id && preg_match('/^(.+)-(\d+)$/', $slug, $m) && (int)$m[2] === $id) {
        return $m[1] !== '' ? $m[1] : ('a' . $id);
    }
    return $slug;
}
...
public function buildAliasesAdSearchCard($data=[]){
    ...
    $slug = $this->normalizeAdSearchAliasForUrl($data);
    $tail = $slug . '-' . (int)$data->id;
    $seg = $this->getAdsSearchUrlSegment();
    $chainDash = $this->getCategoryChainDashForSearchAd((int)($data->category_id ?? 0));

    $geo = null;
    if (!empty($data->geo) && is_object($data->geo) && !empty($data->geo->alias)) {
        $geo = $data->geo;
    } elseif (!empty($data->city_id)) {
        global $app;
        $geo = $app->component->geo->getCityData((int)$data->city_id);
    }
...
/**
 * Проверка пути для карточки объявления на поиск (после снятия префикса языка из сегментов).
 * @return object|null данные getAdSearch() или null
 */
public function matchAdSearchCardUrl($aliasParts, $ad_alias, $ad_id){
    global $app;
    $id = (int)$ad_id;
    if ($id < 1) {
        return null;
    }
    $row = $app->model->ads_search_data->find("id=?", [$id]);
    if (!$row) {
        return null;
    }
    $slugDb = isset($row->alias) ? trim((string)$row->alias) : '';
    if ($slugDb === '') {
        $slugDb = 'a' . $id;
    }
    if ($slugDb !== $ad_alias && $slugDb !== ($ad_alias . '-' . $id)) {
        return null;
    }

    $seg = $this->getAdsSearchUrlSegment();
    $chainDash = $this->getCategoryChainDashForSearchAd((int)$row->category_id);
    $geo = $row->city_id ? $app->component->geo->getCityData((int)$row->city_id) : null;

    if ($geo && !empty($geo->alias)) {
        if ($chainDash === '') {
            if (count($aliasParts) !== 2) {
                return null;
            }
            if ($aliasParts[0] !== $geo->alias || $aliasParts[1] !== $seg) {
                return null;
            }
        } else {
            ...
            if ($aliasParts[0] !== $geo->alias || $aliasParts[1] !== $seg || $aliasParts[2] !== $chainDash) {
                return null;
            }
        }
    } else {
        if ($chainDash === '') {
            ...
            if ($aliasParts[0] !== $seg || $aliasParts[1] !== $chainDash) {
                return null;
            }
        }
    }

    $out = $this->getAdSearch($id);
...
public function buildAdShareModalParams($data){
    $p = ['id' => $data->id];
    if (!empty($data->is_ads_search)) {
        $p['is_ads_search'] = 1;
    }
    return buildAttributeParams($p);
}

/**
 * Ссылка на каталог объявлений «ищу товар».
 */
public function outHrefAdsSearchCatalog(){
    // Путь совпадает с routes/web.php GET /s (именованный маршрут ads-search-catalog).
    return outLink('s');
}

/**
 * Поля формы «ищу товар» (создание / редактирование).
 *
 * @param object|null $adSearch строка из getAdSearch() или null для пустой формы
 */
public function outAdSearchCreateFormFields($adSearch = null){
    global $app;

    return $app->view->setParamsComponent(['adSearch' => $adSearch])->includeComponent('ad-search-create-content.tpl');
}

/**
 * WHERE + params для выборки uni_ads_search_data (каталог).
 *
 * @param array $post как $_POST от формы (filter[...])
 * @param object|null $geo из сессии (city_id / region_id / country_id)
 * @return array{query:string,params:array<int,mixed>}
 */
public function buildAdsSearchListQuery($post = [], $geo = null){
    global $app;

    $query = ['status=?'];
    $params = [1];

    $filter = (isset($post['filter']) && is_array($post['filter'])) ? $post['filter'] : [];
    $switch = (isset($filter['switch']) && is_array($filter['switch'])) ? $filter['switch'] : [];

    if (!empty($switch['delivery']) && empty($switch['city_with_delivery'])) {
        $query[] = 'delivery_shipping_status = ?';
        $params[] = 1;
    }

    $searchRaw = isset($post['search']) ? trim((string)$post['search']) : '';
    if ($searchRaw !== '') {
        $splitData = $app->component->search->splitKeywords($searchRaw);
        $split = [];
        if (!empty($splitData['split']) && is_array($splitData['split'])) {
            foreach ($splitData['split'] as $word) {
        ...
        if ($split) {
            if ($app->settings->search_allowed_text) {
                $itemQuery = $app->component->search->buildKeywordsFields($split, ['search_tags', 'title', 'text']);
            } else {
                $itemQuery = $app->component->search->buildKeywordsFields($split, ['search_tags', 'title']);
            }
            ...
            $like = '%' . $searchRaw . '%';
            if ($app->settings->search_allowed_text) {
                $query[] = '(title LIKE ? OR search_tags LIKE ? OR text LIKE ?)';
                $params[] = $like;
                $params[] = $like;
                $params[] = $like;
            } else {
                $query[] = '(title LIKE ? OR search_tags LIKE ?)';
                $params[] = $like;
                $params[] = $like;
            }
        }
    ...
    if ($geo) {
        $cityId = (int)($geo->city_id ?? 0);
        $regionId = (int)($geo->region_id ?? 0);
        $countryId = (int)($geo->country_id ?? 0);

        if ($cityId) {
            if (!empty($switch['city_with_delivery'])) {
                $query[] = '((city_id = ?) OR (delivery_shipping_status = 1))';
                $params[] = $cityId;
            } else {
                $query[] = '(city_id = ? OR city_id = 0)';
                $params[] = $cityId;
            }
        } elseif ($regionId) {
            $query[] = '(region_id = ? OR region_id = 0)';
            $params[] = $regionId;
        } elseif ($countryId) {
            $query[] = '(country_id = ? OR country_id = 0)';
            $params[] = $countryId;
        }
    }
...
/**
 * Выпадающее меню сортировки для каталога «ищу» (как в каталоге продажи).
 */
public function outAdsSearchCatalogSorting(){
    $sort = $_GET['sort'] ?? 'time';
    if ($sort === 'news') {
...
/**
 * Боковая форма фильтров каталога «ищу товар».
 */
public function outAdsSearchCatalogSidebar($getParams = []){
    global $app;
    if (!is_array($getParams)) {
        $getParams = [];
    }
    return $app->view->setParamsComponent(['filterParams' => $getParams])->includeComponent('ad-search-catalog-sidebar.tpl');
}

public function getAdSearch($id=0, $user_id=0){
    global $app;

    if($user_id){
        $row = $app->model->ads_search_data->find("id=? and user_id=?", [$id,$user_id]);
    }else{
        $row = $app->model->ads_search_data->find("id=?", [$id]);
    }
    ...
    if($catId && isset($app->component->ads_categories->categories[$catId])){
        $data->category = (object)$app->component->ads_categories->categories[$catId];
    }else{
        $data->category = (object)[
            'name' => translate('tr_home_people_search'),
            'seo_title' => '',
            'seo_desc' => '',
            'seo_h1' => '',
            'seo_text' => '',
    ...
    // Как в getAd(): extra_data=true — full_name, avatar_src, shop, service_tariff для карточки и сайдбара.
    $data->user = $app->model->users->findById($data->user_id, true, true);
    ...
    $data->is_ads_search = true;
    $data->booking_status = 0;
    $data->online_view_status = 0;
    $data->price_gratis_status = 0;
    $data->price_fixed_status = 1;
    $data->old_price = null;
    $data->price_measure_id = 0;
    $data->address_latitude = null;
    $data->address_longitude = null;
    $data->partner_link = $data->partner_link ?? null;
    $data->partner_button_name = $data->partner_button_name ?? null;
    $data->partner_button_color = $data->partner_button_color ?? null;
    if(empty($data->currency_code)){
        $data->currency_code = $this->getCurrencyCode($data->currency_code ?? '');
    }

    return $data;
}

public function fixViewSearch($ad_id=0, $user_id=0){
    global $app;

    if($user_id){
        $get = $app->model->ads_search_views->find("user_id=? and ad_id=?", [$user_id, $ad_id]);
        if(!$get){
            $app->model->ads_search_views->insert(["ad_id"=>$ad_id, "user_id"=>$user_id, "ip"=>getIp(), "time_create"=>$app->datetime->getDate()]);
        }
    }elseif(getIp()){
        if(isBot(getUserAgent())){
            return;
        }
        $get = $app->model->ads_search_views->find("ip=? and ad_id=? and date(time_create)=?", [getIp(), $ad_id, $app->datetime->format("Y-m-d")->getDate()]);
        if(!$get){
            $app->model->ads_search_views->insert(["ad_id"=>$ad_id, "ip"=>getIp(), "time_create"=>$app->datetime->getDate()]);
        }
    }
}

public function getViewsSearch($ad_id=0){
    global $app;
    return $app->model->ads_search_views->count("ad_id=?", [$ad_id]);
}

public function getViewsTodaySearch($ad_id=0){
    global $app;
    return $app->model->ads_search_views->count("ad_id=? and date(time_create)=?", [$ad_id,$app->datetime->format("Y-m-d")->getDate()]);
}

public function outBreadcrumbAdSearch(){
    global $app;
    $name = translate('tr_home_people_search');
    $catalogHref = htmlspecialchars($this->outHrefAdsSearchCatalog(), ENT_QUOTES, 'UTF-8');
    return '
        <li class="breadcrumb-item" itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
          <a itemprop="item" href="'.$catalogHref.'"><span itemprop="name">'.htmlspecialchars($name, ENT_QUOTES, 'UTF-8').'</span></a>
          <meta itemprop="position" content="2">
        </li>';
}

public function outActionButtonsInAdSearchCard($data=[]){
    global $app;
...
public function getDataByValue($data=[]){
    ...
    if ($cid && isset($app->component->ads_categories->categories[$cid])) {
        $data["category"] = (object)$app->component->ads_categories->categories[$cid];
    } else {
        $data["category"] = (object)[
            'name' => '',
            'alias' => '',
...
public function getMedia($media=null){
    ...
    $gallery = ["images" => [], "images_m" => [], "video" => []];
    $inline = [];
    $media = is_array($media) ? $media : _json_decode($media);
    ...
    if (is_array($media) && $media !== []) {
        ...
        if (!empty($media["images"]) && is_array($media["images"])) {
            ...
            foreach ($media["images"] as $key => $value) { 
                ...
                if($value["link"]){
                    ...
                    $gallery["images_m"][] = $value["link"];
                }elseif($value["name"]){
                    $originalUrl = $app->storage->name($value["folder"].'/'.$value["name"])->path('market-images')->host(true)->get();
                    $gallery["images"][] = $originalUrl;
                    $nameBase = pathinfo($value["name"], PATHINFO_FILENAME);
                    $mediumPath = $app->config->storage->market->images.'/'.$value["folder"].'/'.$nameBase.'_m.webp';
                    $gallery["images_m"][] = file_exists($mediumPath)
                        ? $app->storage->name($value["folder"].'/'.$nameBase.'_m.webp')->path('market-images')->host(true)->get()
                        : $originalUrl;
                }
        ...
        if (!empty($media["video"]) && is_array($media["video"])) {
        ...
        if (!empty($media["inline"]) && is_array($media["inline"])) {
            foreach ($media["inline"] as $key => $value) {
                if (!is_array($value)) {
                    continue;
                }
        ...
        // Fallback: только inline без массива images (старые/обрезанные записи).
        if (!$gallery["images"] && $inline) {
            foreach ($inline as $item) {
                if (($item["type"] ?? '') === 'image' && !empty($item["link"])) {
                    $gallery["images"][] = $item["link"];
                    $gallery["images_m"][] = $item["link"];
                }
            }
        }
    ...
    $imagesAll = $gallery["images"] ?: [];
    $imagesAllM = $gallery["images_m"] ?: [];
    return arrayToObject([
        "images" => [
            "first" => $imagesAll ? $imagesAll[0] : $app->storage->noImage(),
            "all" => $imagesAll,
            "first_m" => $imagesAllM ? $imagesAllM[0] : ($imagesAll ? $imagesAll[0] : $app->storage->noImage()),
            "all_m" => $imagesAllM,
        ],
        "video" => ["all" => $gallery["video"] ?: []],
        "inline" => $inline ?: [],
        "count" => count($inline),
    ]);
...
public function outActionButtonsInAdCard($data=[]){
    ...
    if(!empty($data->is_ads_search)){
        $this->outActionButtonsInAdSearchCard($data);
        return;
    }
...
public function outShareSocialLinks($id=0, $is_ads_search=false){
    global $app;

    if($is_ads_search){
        $row = $this->getAdSearch((int)$id);
        if(!$row || empty($row->user)){
            return;
        }
        $link = urlencode($this->buildAliasesAdSearchCard($row));
    }else{
        $ad = $this->getAd($id);
            ...
            if($value["type"] == "image"){
                ...
                if($value["name"]){
                    ...
                    if(file_exists($app->config->storage->temp.'/'.$value["name"].'.webp')){
                        ...
                        $destPath = $app->config->storage->market->images.'/'.$folder;
                        if(copy($app->config->storage->temp.'/'.$value["name"].'.webp', $destPath.'/'.$value["name"].'.webp')){
                           $app->image->path($destPath)->name($value["name"].'.webp')->saveTo($destPath)->createMediumCopy(400, 300);
