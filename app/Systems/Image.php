...
class Image
{
    ...
    /**
     * Create a medium copy (e.g. for catalog thumbnails) with suffix _m in the same folder.
     * Does not delete the original. Uses cover() for consistent aspect ratio.
     */
    public function createMediumCopy($width = 400, $height = 300, $position = 'center'){
        ...
        if(isset($this->path) && isset($this->name) && isset($this->saveTo)){
            ...
            if(file_exists($originalPath)){
                ...
                $filenameBase = getInfoFile($this->name)->filename;

                return $this->save($image, null, $this->saveTo, $filenameBase.'_m');
            }
        }
