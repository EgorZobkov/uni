...
public function card($aliases, $ad_alias, $ad_id){   
    ...
    $aliasParts = explode("/", $aliases);
    if($this->translate->getChangeLang() == ($aliasParts[0] ?? '')){
        unset($aliasParts[0]);
        $aliasParts = array_values($aliasParts);
    }

    $searchData = $this->component->ads->matchAdSearchCardUrl($aliasParts, $ad_alias, $ad_id);
    if($searchData){
        return $this->displayAdSearchCard($searchData);
    }
    ...
    if($data->geo){
        if($data->geo->alias != $aliasParts[0] || $chain->chain_build_alias_dash != $aliasParts[1] || $data->alias != $ad_alias){
            abort(404);
        }                
    }else{
        if($chain->chain_build_alias_dash != $aliasParts[0] || $data->alias != $ad_alias){
            abort(404);
        }                 
    }
...
/**
 * Карточка объявления на поиск (тот же шаблон ad-card, что и продажа).
 */
protected function displayAdSearchCard($data){

    $publicOk = ($data->status == 1 && !$data->block_forever_status);
    $owner = $this->user->isAuth() && (int)$data->user_id === (int)$this->user->data->id;
    if(!$publicOk && !$owner){
        abort(404);
    }
    ...
    $this->session->setNestedSubarray('ad-search-contact', $data->id, $data->id);

    $data->in_favorites = false;

    if(!$data->owner){
        $viewerId = $this->user->isAuth() ? (int)$this->user->data->id : 0;
        $this->component->ads->fixViewSearch($data->id, $viewerId);
    }

    $data->count_reviews = 0;
    $data->similar_items = null;

    $page = $this->model->template_pages->find("route_name=?", ['ad-card']);
    if($page){
        $seo = $this->component->seo->content($data, $page->id);
    }else{
        $seo = (object)[
            'meta_title' => $data->title,
            'meta_desc' => mb_substr(strip_tags((string)$data->text), 0, 160),
            'h1' => $data->title,
            'text' => $data->text,
        ];
    }

    return $this->view->render('ad-card', ['data' => $data, 'seo' => $seo]);
}
...
public function changeStatusPublication(){
    ...
    if(!empty($_POST['is_ads_search'])){
        $this->model->ads_search_data->update(["status"=>3], ["id=? and user_id=?", [$id, $this->user->data->id]]);
    }else{
        $this->model->ads_data->update(["status"=>3], ["id=? and user_id=?", [$id, $this->user->data->id]]);
    }
...
public function delete(){
    ...
    if(!empty($_POST["is_ads_search"])){
        $ad = $this->model->ads_search_data->find("id=? and user_id=?", [$id, $this->user->data->id]);
        if($ad){
            $this->model->ads_search_data->delete("id=?", [$id]);
            $this->model->ads_search_views->delete("ad_id=?", [$id]);
            if(isset($this->model->ads_search_filters_ids)){
                $this->model->ads_search_filters_ids->delete("ad_id=?", [$id]);
            }
        }
    }else{
        $this->component->ads->delete($id, $this->user->data->id);
    }
