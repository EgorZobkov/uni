...
{   
    ...
    if (isset($_POST['purpose']) && $_POST['purpose'] === 'all') {
        $this->component->geo->setChange(0, 'all');
        return json_answer(["content"=>""]);
    }

    $result = $this->component->geo->outOptionsCities($_POST['id'] ?? 0);
    ...
    $this->component->geo->setChange($_POST['id'] ?? 0, $_POST['purpose'] ?? '');
...
{   
    ...
    $referrerRaw = isset($_POST['url']) ? (string) $_POST['url'] : '';
    $referrerPath = '';
    if ($referrerRaw !== '') {
        $path = parse_url($referrerRaw, PHP_URL_PATH);
        $referrerPath = trim(($path !== false && $path !== null) ? $path : $referrerRaw, '/');
    }
    $isAdsSearchCatalog = ($referrerPath !== '' && substr($referrerPath, -9) === 'ad/search');

    $url = isset($_POST['geo_alias']) ? trim((string) $_POST['geo_alias'], "/") : '';

    if (!empty($_POST["city_districts"])) {
        $params["city_districts"] = $_POST["city_districts"];
    }

    if (!empty($_POST["city_metro"])) {
    ...
    if ($isAdsSearchCatalog) {

        $this->component->geo->applyAdsSearchCatalogGeoResetIfAllAlias($_POST['geo_alias'] ?? '');
        ...
        $link = outRoute('ads-search-catalog');
        if ($params) {
            $link .= (strpos($link, '?') !== false ? '&' : '?') . http_build_query($params);
        }
        return json_answer(["link" => $link]);
    }
